...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    if dataset=='cpv2':
      ...
      if name == 'train':
        answer_path = './new_data/target.pkl' # ? remember it
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
                 use_hdf5=False, cache_image_features=False, base_score=None):
        ...
        self.tokenize()
        self.tensorize()
        self.base_entries = copy.deepcopy(self.entries)

        if name == 'train':
            self.auto_question_type()

        self.v_dim = 2048
    
    def auto_question_type(self):
        question_types = list(set([x['answer']['question_type'] for x in self.entries]))
        string_to_number_map = {}
        current_number = 1
        for question_type in question_types:
            string_to_number_map[question_type] = current_number
            current_number += 1
        self.question_type2id = string_to_number_map
    ...
    def __getitem__(self, index):
        ...
        if self.image_to_fe is not None:
            ...
            features = torch.load('data/rcnn_feature/' + str(entry["image_id"]) + '.pth', encoding="ISO-8859-1")['image_feature']
        ...
        if self.name == 'train':
            if entry['answer']['labels'] is None:
                entry2 = self.entries[random.choice(self.grouped_data[(None, None)])['serial']]
            else:
                labels = self.base_entries[entry['serial']]['answer']['labels'].tolist()
                scores = self.base_entries[entry['serial']]['answer']['scores'].tolist()

                filtered_labels = [label for label, score in zip(labels, scores) if score > self.base_score]
                filtered_scores = [score for label, score in zip(labels, scores) if score > self.base_score]
                ...
                key = (labels, scores)
                # if entry['answer']['question_type'].__contains__('color'):
                #     if self.grouped_data[key].__len__() > 5:
                #         print([x['question_id'] for x in self.grouped_data[key]])
                # sys.eixt(-1)
                entry2 = self.entries[random.choice(self.grouped_data[key])['serial']]
                
            if self.image_to_fe is not None:
                features2 = self.image_to_fe[entry2["image_id"]]
            elif self.use_hdf5:
                features2 = np.array(self.features[entry2['image_idx']])
                features2 = torch.from_numpy(features).view(36, 2048)
            else:
                features2 = torch.load('data/rcnn_feature/' + str(entry2["image_id"]) + '.pth', encoding="ISO-8859-1")['image_feature']
            ques2 = entry2['q_token']
            ...
            # if entry['question'] == 'What color is the kite?':
            #     print(entry['image_id'])
            #     print(entry2['image_id'])
            # if entry['question_id'] != entry2['question_id']: print('===')
            return index, features, ques, target, entry['bias'], features2, ques2, target2, entry2['bias']
        # if self.name=='train':
        ...
        #     if "bias" in entry:
        #         return index, features, ques, target,entry["bias"], train_hint, type_mask, notype_mask, ques_mask
        #     else:
        #         return index, features, ques,target, 0, train_hint
        else:
