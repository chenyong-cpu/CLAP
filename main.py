...
from label import labeling_for_label_score
...
def parse_args():
    ...
    parser.add_argument('--endepoch', type=int, default=5)
    ...
    parser.add_argument('--stop_many', type=int, default=2)
...
def get_bias(train_dset,eval_dset):
    ...
    for ex in eval_dset.entries:
        ex['bias'] = np.zeros(answer_voc_size, np.float32)
    
    # 问题类型
    for ex in train_dset.entries:
        if ex['answer']['question_type'].__contains__('many') or ex['answer']['question_type'].__contains__('number'):
            ex['type'] = 'number'
        elif np.argmax(ex['bias']) in [train_dset.ans2label['yes'], train_dset.ans2label['no']]:
            ex['type'] = 'yes/no'
        else:
            ex['type'] = 'other'
...
def main():
    ...
    train_dset = VQAFeatureDataset('train', dictionary, dataset=dataset,
                                   cache_image_features=True, base_score=args.score)
    ...
    # single_bias(train_dset, eval_dset)
    labeling_for_label_score(train_dset, base_score=args.score)
